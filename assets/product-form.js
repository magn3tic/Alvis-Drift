      ...
      onSubmitHandler(t) {
        ...
        fetch(`${routes.cart_add_url}`, r)
          ...
          .then((t) => {
            ...
            // ✅ Update bubble with total price after add-to-cart
            if (t.total_price !== undefined) {
              $(".cart-count-bubble > span").text(formatMoney(t.total_price));
            }
          })
          .catch((t) => {
            console.error(t);
...
// ✅ Helper: format cents to currency string (e.g. 1999 → "R19.99")
...
function updateQty(qty, lineId) {
  ...
  timeoutHandle = setTimeout(function () {
    // ✅ Use $.getJSON so cart response is auto-parsed as JSON
    $.getJSON(
      "https://alvisdrift.co.za/cart/change?id=" + lineId + "&quantity=" + qty,
      function (cart, status) {
        if (status === "success") {
          $("#mini-cart_popup").load(
            location.href + " #mini-cart_popup_load",
            function () {
              // ✅ Show total_price instead of item count
              $(".cart-count-bubble > span").text(formatMoney(cart.total_price));
            }
          );
        }
      }
